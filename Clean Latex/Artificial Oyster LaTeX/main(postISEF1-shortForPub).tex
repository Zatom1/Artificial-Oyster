...
\Archive{International Science and Engineering Fair} % Additional notes (e.g. copyright, DOI, review/research article)
...
\Keywords{Microplastics --- Fluorescence --- Selective Filtration --- Benthic} % Keywords - if you don't want any simply remove all the text between the curly brackets
...
\Abstract{Since the invention of modern plastics during the 20th century, microplastics (MPs) have become one of the most pervasive anthropogenic pollutants on Earth. Very few methods have been proposed to remove these plastics in-situ from the environment, and to the author's knowledge no methods have been proposed to remove them from the benthic region, a place where they can have immense negative impacts by acting as vectors for persistent organic pollutants, deoxygenating sediment, and by adversely affecting various biological processes in benthic organisms. This paper thus presents one of the first techniques capable of separating these plastics within the scope of river ecosystems, and furthermore presents a first step towards creating a generalized system capable of removing any class of particle from any riverine ecosystem; a task which thus far been reliant on expensive and ecologically damaging dredging. The device developed here operates by illuminating suspended particles with laser light, measuring the reflected light with a camera, then either collecting or releasing measured particles back into the river depending on the reflected light's wavelength and intensity. Empirical results from testing in the Milwaukee River have shown this method to remove the vast majority of microplastics which pass through the device, with two preliminary tests showing 90.54\% and 92.24\% ($\pm 32.27\%$) removal efficiencies. With more development, the technique used here to collect microplastics could be applied to other classes of particle pollutant in rivers, presenting the first step towards a low-cost, generalizable, and self-sufficient method of particle removal in rivers which could be applied for significant ecological benefit.}
...
\begin{document}
	...
	Since their discovery in 2004, microplastics (MPs) have widely spread and now permeate nearly every region and ecosystem in the world. Of particular concern in this regard are organisms within the \gls{benthic} region of waterways, as these areas’ typically close proximity to human settlement increases the rate of microplastic entry into their habitats\textemdash and the lowered rate of microplastic transport within the \gls{benthic} layer promotes accumulation of MPs. This accumulation is especially pernicious, as the \gls{benthic} zone contains hundreds of species of filter feeders (notably bivalves like oysters, mussels, and clams) which are crucial in the upkeep of waterway health and in preventing the eutrophication of aquatic environments. These bivalves are highly effective at separating the sediment, nutrients, and particulates carried in \gls{benthic} currents, however, the increase in suspended solids caused by microplastic pollution combined with the large volumes of water filtered by bivalves has caused significant damage to bivalve populations worldwide. This damage takes many forms, with microplastics having been found accumulating in bivalves' digestive tracts, decreasing bivalves' efficiency at absorbing oxygen, and acting as a vector for persistent organic pollutants (POPs) like DDT to be consumed at high rates. Thus, in addition to removing \gls{pelagic} microplastics, actively removing \gls{benthic} microplastics is key to successfully combating the problems of microplastic pollution which currently plague the world’s lakes, rivers, and oceans. Thus far, no solution has been found to this problem, and microplastics have continued to accrue in the sediment of global waterways\textemdash thus creating a device capable of reliably, autonomously, and non-disruptively extracting these microplastics is necessary to ensure the future health of these critical ecosystems. This project targets rivers in particular for microplastic removal due to their critical role in transporting microplastics into lakes and oceans. The rivers of the Laurentian Great Lakes basin serves as a model, given the basin’s importance. These lakes  and rivers contain nearly ¼ of the entire world’s supply of liquid freshwater and support 10s of millions of people, making them one of the most significant  sources of freshwater in the world. In this project, the Milwaukee River serves as the area of experimentation, chosen due to the large set of existing data available on microplastic concentrations in the river. By significantly reducing the quantity of microplastics, three primary goals should be achieved in the long term, those being: 
	\begin{enumerate}
	...
	\end{figure}
	This, notably, means that the proposed device could be used not only to remove microplastics, but any other particle class with distinct fluorescence spectra\textemdash though separation with the exact device created here is likely to be limited to microplastics or other similarly distinctive particles due to relatively low camera resolution. After taking an image, the camera’s data can then be processed by a microcomputer such as a Raspberry Pi in order to identify the particles that exist within the sample, and make a decision as to whether or not the sample should be filtered. As aforementioned, it is all but impossible to accurately, continuously, and selectively filter only one type of particle from a sample. To circumvent this problem, the device presumes that each particle it detects lies within a discrete volume, and only filters this volume when microplastics are determined to be the majority of detectable particles in the sample. This does mean that any particles within the volume which are not microplastics will be removed, and thus there will almost inevitably be some non-microplastic particles collected, however, given the low amount of water being analyzed in any given volume (4500 mm$^{3}$, 0.0045 liters) elementary calculations show us that this will only be a very significant problem during outlier events with very high total suspended solids. \linebreak
	
	Thus, we are able to discretely filter particles by diverting the fluid flow into one of two channels\textemdash with one channel exiting the device and returning particles back into the flow of the river, and the other leading particles through an indiscriminate filter which removes all suspended matter above its pore diameter from the channel. 
	...
	The most complex of the sections is the processor, and it handles all of the work of identifying particles in tandem with the compute module. A cross-section of one of the processor's two laser modules is shown in figure \ref{fig:ProcessorCrosssec}, with the 30x30mm square tube at the bottom-center, the two 405 nm lasers placed in the angled cavities at the top, and the  \gls{RPI} Camera Module V2 looking down through the channel. The lasers shine through a thin 8mm flat cavity, angled such that their 110°-wide beams shine directly downwards and outwards to avoid intrusion into the 6mm wide cavity through which the cameras view the tube, and illuminate a thin section\textemdash causing each particle to re-emit light at a certain intensity and wavelength. 
	\begin{figure}[h]
		\centering
	...
	\end{figure}
	This is then captured by the camera, which captures footage at roughly 215 fps to ensure capture of particles traveling at speeds as high as 2 m/s. The data captured by the cameras is then transferred to the \gls{RPI} compute module for processing\textemdash after which the \gls{RPI} sends commands to the servo housed within the selector section to select which particles should be filtered and which should be expelled. 
	\begin{figure}[h]
		\centering
		\includegraphics[width=0.5\linewidth]{Figures/OverheadCrosssection}
		\caption[Selector Cross-section]{An overhead cross-section of the processor and selector. The processor can be seen as the two bumps which extrude from the side of the channel near the top of the figure, and the selector's flipper is shown in gray near the bottom.}
		\label{fig:OverheadCrosssec}
	\end{figure}
	
	\subsubsection{Selector}
	The selector is a relatively simple mechanism attached directly to the outlet which consists of a single servo motor attached to a flipper which controls the direction which water travels. As shown, the gray flipper rotates on the servo, with figure \ref{fig:OverheadCrosssec} showing the flipper in the default position which allows water to travel through the right-side channel and back out to the river without disruption. If switched to allow water to flow through the left-side channel, the water will travel through a filter and collection mechanism, removing any diverted \glspl{seston} from the flow.
	
	\subsubsection{Outlet}
	Nearly as simple as the inlet, the outlet consists of two channels which transfer water out of the device and expel all non-diverted \glspl{seston} back into the general river flow.
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=1\linewidth]{Figures/ProcessorMk2}
		\caption[Processor Tech. Drawing]{Technical drawing of the processor.}
		\label{fig:techprocessor}
	\end{figure}
	The processor is the most complex portion of the device, being constructed of two laser modules, each with two halves which are bound together with epoxy and physical connectors to ensure a strong connection. The processor contains the laser modules, each with two lasers which, together, illuminate the particles travelling through the device. The lasers illuminate each particle from both sides equally, enabling the cameras to have a clear view of each particle. As seen in the technical drawing, the cavity through which the cameras view the particles is only 6mm wide, ensuring that every capture has a very similar profile and that the linear lasers always light up the particles in the viewport. Near the bottom, on the left and right sides, there also exist connection points for the camera’s waterproof enclosure to ensure water doesn’t enter the cameras’ compartment and risk damaging them. To aid in this, the cameras’ circuit boards are also covered in epoxy so that any minor leaks, humidity, or other water intrusion doesn’t damage them. 
	\begin{figure}[h]
		\centering
...
%	\end{table}
	...
	\begin{figure}[h]
		...
		\caption[Device Deployed into River]{Once placed into the river, the device was left in place with the ropes tied to a tree on the river's bank.}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=1\linewidth]{Figures/PercentDetectable}
	...
	\begin{enumerate}
		...
		\item The river must not flow more quickly\footnote{This restriction is a result of the device's computing limitations. Given the roughly 6 mm wide aperture through which the cameras can image microplastics, and the rate of image capture and processing (230x per second max), any speeds above 2 m/s allow for the possibility of particles being completely unseen by the device. As speeds approach 2 m/s, it is theorized that the device's effectiveness may asymptotically decline as the cameras obtain fewer and fewer images of the particles with which to detect MPs, though this has not been empirically tested.} than $\approx$ 2 m/s
	...
	\begin{figure}[h]
		\centering
		\includegraphics[angle=0,width=1\linewidth]{Figures/RawSeparated}
